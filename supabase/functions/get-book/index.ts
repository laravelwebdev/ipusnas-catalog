...
import { decrypt, resolvePassKey, resolvePassZip, resolvePassPdf } from "../_shared/decrpt.ts";
import { corsHeaders } from "../_shared/cors.ts";
async function checkProfile(query) {
  ...
  const base = `${BASE_URL}/webhook/profile`;
...
async function getBook(query) {
  ...
  const base = `${BASE_URL}/webhook/book-detail`;
  const reqUrl = `${base}?${queryParams.toString()}`;
  return await fetch(reqUrl, {
    headers: {
      "user-agent": USER_AGENT
